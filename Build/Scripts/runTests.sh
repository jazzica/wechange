...
read -r -d '' HELP <<EOF
wechange test runner. Execute unit test suite and some other details.
...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/wechange/${1}"
else
    case ${TEST_SUITE} in
        acceptance)
            TEST_FILE="Web/typo3conf/ext/wechange/Tests/Acceptance"
            ;;
        functional)
            TEST_FILE="Web/typo3conf/ext/wechange/Tests/Functional"
            ;;
        unit)
            TEST_FILE="Web/typo3conf/ext/wechange/Tests/Unit"
            ;;
    esac
fi
